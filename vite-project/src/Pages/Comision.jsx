...
const Comision = () => {
  ...
  const obtenerDatos = async () => {
    try {
      ...
      const [comisionesRes, usuariosRes, reunionesRes, asistenciasRes] =
        await Promise.all([
          axios.get("http://127.0.0.1:5000/api/comisiones"),
          axios.get("http://127.0.0.1:5000/api/usuarios"),
          axios.get("http://127.0.0.1:5000/api/reuniones"),
          axios.get("http://127.0.0.1:5000/api/asistencias"),
        ]);
      ...
      if (reunionesRes.data.length > 0) {
        setReunionSeleccionada(reunionesRes.data[0]._id);
      }
    } catch (error) {
      console.error(error);
  ...
  // GUARDAR ASISTENCIA
  const guardarAsistencia = async (usuarioId, reunionId, estado) => {
    if (!reunionId) {
      alert("Selecciona una reunión");
    ...
    try {
      ...
      const asistenciaExistente = asistencias.find(
        (a) => a.usuarioId === usuarioId && a.reunionId === reunionId,
      );
      ...
      if (asistenciaExistente) {
        ...
        // SI NO EXISTE -> CREATE
        await axios.post("http://127.0.0.1:5000/api/asistencias", {
          usuarioId,
          reunionId,
          estado,
          fecha: new Date(),
  ...
  // CONTAR PRESENTES
  const obtenerPresentes = (reunionId) => {
    return asistencias.filter(
      (a) => a.reunionId === reunionId && a.estado === "Presente",
    ).length;
  };
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="ml-64 p-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-6 py-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Comisiones</h1>

            <p className="text-slate-500 mt-1">
              Gestión de asistencia y seguimiento de reuniones
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-500 text-sm">Comisiones activas</p>

                <h2 className="text-3xl font-bold text-slate-800 mt-2">
                  {comisiones.length}
                </h2>

                <p className="text-sm text-slate-400 mt-2">Registradas</p>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-500 text-sm">Reuniones</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-8">
          <div className="px-6 py-5 border-b border-slate-200">
            ...
            <p className="text-sm text-slate-500 mt-1">Control de reuniones</p>
          </div>
            ...
            <div className="min-w-[1200px]">
              ...
              {reuniones.map((reunion) => (
                ...
                >
                  ...
                  <div className="col-span-2">{reunion.fecha}</div>

                  <div className="col-span-2">{reunion.tipo}</div>

                  <div className="col-span-2">{reunion.estado}</div>

                  <div className="col-span-2">
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">
                      {obtenerPresentes(reunion._id)}
                      {" / "}
                      {totalUsuarios}
                    </span>
                  </div>
                  ...
                  <div className="col-span-2">
                    <button
                      onClick={() => setReunionSeleccionada(reunion._id)}
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[1300px]">
              ...
              <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-slate-50 border-b border-slate-200 text-sm font-semibold text-slate-600 uppercase">
                ...
                <div className="col-span-2">Claustro</div>
              ...
              {usuarios.map((usuario) => {
                const asistenciaUsuario = asistencias.find(
                  (a) =>
                    a.usuarioId === usuario._id &&
                    a.reunionId === reunionSeleccionada,
                );
                    ...
                    <div className="col-span-2">
                      <span
                        className={`
                            ...
                            font-semibold
                            ${
                              asistenciaUsuario?.estado === "Presente"
                                ? "bg-green-100 text-green-700"
                                : asistenciaUsuario?.estado === "Ausente"
                                  ? "bg-red-100 text-red-700"
                                  : "bg-slate-100 text-slate-700"
                            }
                          `}
                      >
                        {asistenciaUsuario?.estado || "Sin registrar"}
                      </span>
                    </div>
