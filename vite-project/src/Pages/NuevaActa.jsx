...
const NuevaActa = () => {
  ...
  const guardarActa = async (e) => {
    ...
    try {
      ...
      await axios.post("http://localhost:5000/api/actas-reunion", formulario);
  ...
  // TEMAS SELECCIONADOS
  const temasSeleccionadosObjetos = temasDisponibles.filter((t) =>
    formulario.temas.includes(t._id),
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        "
        >
          <form onSubmit={guardarActa} className="space-y-8">
            {/* GRID */}
            <div
              className="
              grid
